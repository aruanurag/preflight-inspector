	...
	return func(r *v1beta1.AdmissionRequest) (*preflightinspector.Result, error) {
		...
		for _, c := range pod.Spec.Containers {
			if strings.HasSuffix(c.Image, ":latest") {
				return &preflightinspector.Result{Msg: "You cannot use the tag 'latest' in a container."}, nil
			}
		}
...
func mutateCreate() preflightinspector.AdmitFunc {
	return func(r *v1beta1.AdmissionRequest) (*preflightinspector.Result, error) {
		var operations []preflightinspector.PatchOperation
		pod, err := parsePod(r.Object.Raw)
		if err != nil {
			return &preflightinspector.Result{Msg: err.Error()}, nil
		}
		...
		if pod.Namespace == "special" {
			...
			operations = append(operations, preflightinspector.ReplacePatchOperation("/spec/containers", containers))
		}

		// Add a simple annotation using `AddPatchOperation`
		metadata := map[string]string{"origin": "fromMutation"}
		operations = append(operations, preflightinspector.AddPatchOperation("/metadata/annotations", metadata))
		return &preflightinspector.Result{
			Allowed:  true,
			PatchOps: operations,
		}, nil
	}
